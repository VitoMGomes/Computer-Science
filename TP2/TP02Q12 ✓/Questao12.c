...
int selecaoNomeNascimento(Jogador* jogador1, Jogador* jogador2) {
    if (jogador1->anoNascimento < jogador2->anoNascimento) {
        return -1;
    } else if (jogador1->anoNascimento > jogador2->anoNascimento) {
        return 1;
    } else {
        // Anos de nascimento são iguais, compare os nomes
        int cmp = strcmp(jogador1->nome, jogador2->nome);
        return cmp;
    }
}
...
{ 
    for(int i=0; i < tam; i++)
    {
        for(int j = tam; j>i; j--)
        {
            ...
            else if (selecaoNomeNascimento(jogador[j], jogador[j-1]) < 0)
            {
                swap(jogador, j - 1 , j);
                movimentacoes += 3;
                comparacoes += 2;
